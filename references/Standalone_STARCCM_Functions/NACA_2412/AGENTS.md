...
```mermaid
flowchart TD
    F1["NACA_2412.java / NACA_2412.execute(): 文件入口"]
    F1 --> F1M["方法链：execute()"]
    F1M --> F1A["实际调用：execute(), getActiveSimulation(), createSketch(), createSketchPoint(), createSpline(), createPart(), partTranslate(), partSubtract(), RegionBuilder(), part2Region(), create..."]
    F1A --> F1O["对象/API 类型：Sketch、PhysicsContinuum、Simulation、StarMacro、Scene、Report、Monitor"]
    F1O --> F1S["状态变化：execute()、createSketch()、createSketchPoint()、createSpline()、createPart()、createPhysicsContinua()、enable()、setCourantNumber()、deleteBoundary()、setBoundaryCondition()、setInit..."]
    F1S --> F1R["名称/路径/参数：NACA 2412、XY、Front、LowerSurface、UpperSurface、TrailingEdge_0、TrailingEdge_1、Back"]
