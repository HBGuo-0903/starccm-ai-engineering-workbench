...
```mermaid
flowchart TD
    F1["PhiStudyLes.java / PhiStudyLes.execute(): 文件入口"]
    F1 --> F1M["方法链：execute() → getFileName() → saveAs() → getRpmString() → getRotorDiameterString() → StartRstReactiveSimulation() → setReactionFieldFunctions() → RecordOutletConcentrations() ..."]
    ...
    F1A --> F1O["对象/API 类型：Simulation、StarMacro、FieldFunction、Region、Boundary、Plot、Monitor、MonitorPlot、ReportManager、Report、UserFieldFunction、PhysicsContinuum"]
    ...
    F1C1983709047 --> F1R["名称/路径/参数：Physics 1、Fluid、OutletA、OutletB、Fluid.Container.Wall、mSO4_2-、mNa_1+、mBa_2+"]
