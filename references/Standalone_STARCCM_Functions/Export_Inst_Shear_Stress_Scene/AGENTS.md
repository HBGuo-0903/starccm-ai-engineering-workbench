...
本宏通过在 STAR-CCM+ 中执行 ExportInstShearStressScene.execute()，并调用 execute(), getActiveSimulation(), createShearStressScene(), createEmptyScalarScene(), getScene(), createSubFolder(), getDisplayerManager(), getDisplayer() 操作 StarMacro、Simulation、Scene、Legend、FieldFunction、Displayer、Region、Boundary，实现将 simulation 中的结果或监视数据整理并导出为可复用文件，解决结果文件和监视数据需要人工整理。

## Input

- 已打开的 STAR-CCM+ simulation，以及代码中通过名称获取的已有对象。
- 代码中引用的对象名称或参数：Instantaneous Shear Stress、images、WallShearStress、Stator、Fluid、RPM、, 、).replace(。运行前需确认模型树中名称一致。

## Output

- 被创建、读取或修改的 STAR-CCM+ 对象：StarMacro、Simulation、Scene、Legend、FieldFunction、Displayer、Region、Boundary。
- 代码执行的结果操作：execute()、createShearStressScene()、createEmptyScalarScene()、createSubFolder()、initialize()、addVectorFieldFunctionMagnitudeToScalarDisplayer()、addBoundaryToDisplayer()。

## Workflow

```mermaid
flowchart TD
    F1["ExportInstShearStressScene.java / ExportInstShearStressScene.execute(): 文件入口"]
    ...
    F1O --> F1S["状态变化：execute()、createShearStressScene()、createEmptyScalarScene()、createSubFolder()、initialize()、addVectorFieldFunctionMagnitudeToScalarDisplayer()、addBoundaryToDisplayer()"]
    F1S --> F1C27574779{"控制流：if (recordScene) {"}
    F1C27574779 --> F1C1773185267{"控制流：try {"}
    F1C1773185267 --> F1R["名称/路径/参数：Instantaneous Shear Stress、images、WallShearStress、Stator、Fluid、RPM、, 、).replace("]
