...
本宏通过在 STAR-CCM+ 中执行 Props_v0Fix.execute()，并调用 execute(), initMacro(), setSpeed(), setHeight(), setTrim(), setCsys(), setRpm(), run() 操作 Displayer、StarMacro、Simulation、MacroUtils、MeshOperationManager、MonitorPlot、Scene、UserDeclarations，实现在 STAR-CCM+ simulation 中执行该功能对应的对象配置和结果处理，解决该类 STAR-CCM+ 模型处理依赖重复手工操作。

## Input

- 已打开的 STAR-CCM+ simulation，以及代码中通过名称获取的已有对象。
- 代码中引用的对象名称或参数：6036_v、Revision、Speed (mph)、Trim (deg)、Height (in.)、RPM、Prop Lift (lbf)、Prop Sideforce (lbf)。运行前需确认模型树中名称一致。
...
```mermaid
flowchart TD
    F1["Props_v0Fix.java / Props_v0Fix.execute(): 文件入口"]
    ...
    F1C1432622892 --> F1R["名称/路径/参数：6036_v、Revision、Speed (mph)、Trim (deg)、Height (in.)、RPM、Prop Lift (lbf)、Prop Sideforce (lbf)"]
