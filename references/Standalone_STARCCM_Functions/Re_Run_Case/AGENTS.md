...
```mermaid
flowchart TD
    F1["reRunCase.java / reRunCase.execute(): 文件入口"]
    F1 --> F1M["方法链：execute() → executeWrapAndMesh() → postProcessing()"]
    F1M --> F1A["实际调用：execute(), clearSolutionHistory(), executeWrapAndMesh(), runCase(), saveSimWithSuffix(), postProcessing(), genVolumeMesh(), hardCopyPictures()"]
    F1A --> F1O["对象/API 类型：MacroUtils"]
    F1O --> F1S["状态变化：execute()、clearSolutionHistory()、executeWrapAndMesh()、runCase()、saveSimWithSuffix()、genVolumeMesh()、hardCopyPictures()"]
