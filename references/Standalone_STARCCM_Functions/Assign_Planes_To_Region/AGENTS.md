...
```mermaid
flowchart TD
    F1["AssignPlanesToRegion.java / AssignPlanesToRegion.execute(): 文件入口"]
    F1 --> F1M["方法链：execute() → AssignPlanesToRegion() → execute0()"]
    F1M --> F1A["实际调用：execute(), execute0(), AssignPlanesToRegion(), getActiveSimulation(), getPartManager(), getObject(), getInputParts(), setQuery(), getRegionManager(), getRegion(), getBounda..."]
    F1A --> F1O["对象/API 类型：StarMacro、Simulation、Plane、Region、Boundary"]
    F1O --> F1S["状态变化：execute()、execute0()、AssignPlanesToRegion()、setQuery()、setObjects()"]
    F1S --> F1R["名称/路径/参数：XY Plane、Fluid、InletA、InletB、OutletA、OutletB、Rotor、Stator"]
