...
```mermaid
flowchart TD
    F1["Jpo_Boat_Para_SinglePhase_FrontArea.java / Jpo_Boat_Para_SinglePhase_FrontArea.execute(): 文件入口"]
    F1 --> F1M["方法链：execute() → initMacro()"]
    F1M --> F1A["实际调用：execute(), getSimulation(), getSessionDir(), initMacro(), post(), byREGEX(), getNormalCoordinate(), setCoordinate(), createSheet(), createRow(), createCell(), setCellValue()"]
    F1A --> F1O["对象/API 类型：StarMacro、Simulation、MacroUtils、UserDeclarations"]
    F1O --> F1S["状态变化：execute()、setCoordinate()、createSheet()、createRow()、createCell()、setCellValue()、write()、close()、create()"]
    ...
    F1C1787382376 --> F1R["名称/路径/参数：310slx_hydro、_sink、_roll、_pitch、_yaw、_speed、\\、.sim"]
